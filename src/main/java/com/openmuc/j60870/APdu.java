...
public class APdu {
    ...
    public static APdu decode(Socket socket, ConnectionSettings settings) throws IOException {
        ...
        /*
        int avalible = is.available();
        System.out.println(FIRST_BYTE + " avalible:"+ avalible + "length:"+length);
        if (FIRST_BYTE != START_FLAG || length<=0) {
            String bytes = "";
            byte[] buffer = new byte[avalible];
            is.readFully(buffer,0,avalible);
            for(int i=0;i<avalible;i++){
                bytes=bytes+buffer[i]+",";
            }
            System.out.println("exception:"+bytes);
            throw new IOException("Message does not start with START flag (0x68). Broken connection.");
        }
